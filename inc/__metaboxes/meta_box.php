...
define( 'CUSTOM_METABOXES_DIR', PLANA . 'inc/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'color':
			...
			$meta = $meta ? $meta : '';
			$out .= '<input type="text" class="cr_iris_color" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $meta . '" size="10" />
				<br />' . $desc;
			$out .='<script type="text/javascript">
				jQuery(function($) {
					$(".cr_iris_color").wpColorPicker();
				});
				</script>';
			return $out;
		break;
		...
		case 'post_chosen':
			...
			echo '</select> &nbsp;<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">'. __('Выберете','wp_panda') . ' ' . $post_type_object->label ) . '</a></span><br />' . $desc;
		break;
		// post_checkboxes
		case 'post_checkboxes':
			...
			$post_type_object = get_post_type_object( $post_type );
			echo '</ul> ' . $desc , ' &nbsp;<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">'. __('Выберете','wp_panda') . ' ' . $post_type_object->label ) . '</a></span>';
		break;
		// post_drop_sort
		case 'post_drop_sort':
			//areas
			$post_type_object = get_post_type_object( $post_type );
			echo '<p>' . $desc . ' &nbsp;<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">'. __('Выберете','wp_panda') . ' ' . $post_type_object->label ) . '</a></span></p><div class="post_drop_sort_areas">';
		...
		case 'audio':
			$out='';		
			//$iconClass = 'meta_box_file';
			$out .= "<i class='dashicons dashicons-format-audio label-icon'></i>";
			//if ( $meta ) $iconClass .= ' checked';
			$out .='<div class="meta_box_audio_stuff"><input name="' . esc_attr( $name ) . '" type="text" class="meta_box_upload_audio" value="' . esc_url( $meta ) . '" />
						<div class="clear"></div>';
			if ( $meta ) { 
				$out .= '<span class="meta_box_audio_name">' . do_shortcode('[audio src='. esc_url( $meta ) .']' ) . '</span><div class="clear" style="margin-bottom:20px;"></div>';
			} else {
				$out .= '<span class="meta_box_audio_name"></span><div class="clear" style="margin-bottom:20px;"></div>';
			}
						$out .= '<a href="#" class="meta_box_upload_audio_button button button-primary button-large" rel="' . get_the_ID() . '">' . __( 'Загрузить трек', 'wp_panda' ). '</a>
							<small>&nbsp;<a href="#" class="meta_box_clear_audio_button dashicons dashicons-dismiss">'.__('','wp_panda').'</a></small></div>
							<br clear="all" />' . $desc;
			return $out;
			print_r( $wp_styles );
		break;
