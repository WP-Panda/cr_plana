jQuery(function($) {
	...
	$('.meta_box_upload_file_button').click(function(event) {
		...
		fileFrame = wp.media({
			...
			multiple: false,
			library: {
		 		//type: 'audio'//'image'//audio, video, application/pdf, ... etc
			},
			button: {
		  		text: 'Use This File'
			}
	...
	// the file image button, saves the id and outputs the file name
	var audioFrame;
	$('.meta_box_upload_audio_button').click(function(event) {
		event.preventDefault();
		
		var options, attachment;
		
		$self = $(event.target);
		$div = $self.closest('div.meta_box_audio_stuff');
		...
		audioFrame = wp.media({
			...
			button: {
		  		text: 'Вставить этот трек'
			}
		});
		
		// set up our select handler
		audioFrame.on( 'select', function() {
			selection = audioFrame.state().get('selection');
			...
			selection.each( function( attachment ) {
				...
				$div.find('.meta_box_upload_audio').val(src);
				$div.find('.meta_box_audio_name').html('<audio src="' + src + '">');
				$('video,audio').mediaelementplayer(/* Options */);
				/*var player_selector = $div.find('.meta_box_audio_name');
				var player = new MediaElementPlayer( player_selector );
				// ... more code ...
				player.pause();
				player.setSrc(src);
				player.play();*/
				$div.find('.meta_box_audio').addClass('checked');
			} );
		});
	...
	// the remove image link, removes the image id from the hidden field and replaces the image preview
	$('.meta_box_clear_audio_button').click(function() {
		$(this).parent().siblings('.meta_box_upload_audio').val('');
		$(this).parent().siblings('.meta_box_audio_name').text('');
		$(this).parent().siblings('.meta_box_audio').removeClass('checked');
		return false;
	});
